...
""" SC 12Oct2017                                 """
""" This is visualisation script offers 3 options""" 
""" Energy, spectrum or scroll                   """ 
...
def microphone_update(audio_samples, vizLeds,viz_effect, min_freq,max_freq,FFT_Bins):
    global y_roll, prev_rms, prev_exp, prev_fps_update
    config.VISUAL_EFFECT = viz_effect    
    ...
    if vol < config.MIN_VOLUME_THRESHOLD:
        ...
        # ORIG  output = visualization_effect(mel)

        output = globals()[config.VISUAL_EFFECT](mel)       # Added 12Oct2017, allows for configuration to define effect mode
        led.pixels = output
        led.update()
        if config.USE_GUI:
...
""" SC 12Oct2017, changed to allow passing of effect  from config file """ 
                                                    
visualization_effect = config.VISUAL_EFFECT
